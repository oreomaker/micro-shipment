...
import com.wgzmb.config.Constants;
...
//    private static HashMap<String, String> userInfo = new HashMap<>();
    ...
    public static void verify(String token) {
        ...
        String id = JwtToken.decode(token).getClaim("id").asString();
        String username = JwtToken.decode(token).getClaim("username").asString();
        String email = JwtToken.decode(token).getClaim("email").asString();
        String role = JwtToken.decode(token).getClaim("role").asString();
        UserInfo.set("id", id);
